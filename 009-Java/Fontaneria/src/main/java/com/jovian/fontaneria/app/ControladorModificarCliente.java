...
//*******************************************************************************************************************************************************
	...
	@FXML public void comprobarBusquedaDni(ActionEvent event) throws SQLException {
		...
		if(correcto)leerDatos("buscarCliente");
	...
	private void leerDatos(String metodo) throws SQLException {
		...
		try {
			...
			if(conectado) {
				...
				if(metodo.equals("buscarCliente"))buscarCliente();
				else if(metodo.equals("modificarDatos"))modificarDatos();
			}
...
//*******************************************************************************************************************************************************
	...
	private void buscarCliente() throws SQLException {
		...
		if(btnId.equals("ningunBoton")) {
			...
			btnBuscarDni.setDisable(true);
			btnBuscarNombre.setDisable(true);
			btnModificarDatos.setDefaultButton(false);
			...
			while(rs.next()) {
				...
				//activamos el boton para poder modificar
...
//*******************************************************************************************************************************************************
	...
	@FXML public void modificarCliente(ActionEvent event) throws SQLException {
		...
		//iniciamos la comprobación de los datos del cliente
		else correcto = Chequeable.chequeaCliente(lblWarning,tfDNI, tfNombreCliente,tfApellido1, tfApellido2, tfDireccion, tfCPostal, tfLocalidad, tfProvincia, tfEmail, tfTelefono);
		
		//si está todo correcto, conectamos con la BBDD e insertamos el cliente
		if(correcto)leerDatos("modificarDatos");
	}
